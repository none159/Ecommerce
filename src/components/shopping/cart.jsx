...
const Cartpage = ()=>{
    ...
    const fetchcart = async()=>{
        ...
        const usercart = await axios.post("http://127.0.0.1:5000/api/users/cart",{"email":email}).then((response)=>{return response.data})
        usercart.map(async(cart)=>{
         await axios.get(`http://127.0.0.1:5000/api/product/${cart.productid}`).then((response)=>{
